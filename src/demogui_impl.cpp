...
Dlg::Dlg(wxWindow *parent, demo_pi *ppi)
	: m_Dialog(parent)
{
	this->Fit();
	...
	wxString blank_name = *GetpSharedDataLocation()
		+ _T("plugins/demo_pi/data/blank.ico");
...
failed:
    ...
    wxMessageDialog mdlg(this, error, _("demo"), wxOK | wxICON_ERROR);
    mdlg.ShowModal();
...
void Dlg::Calculate( wxCommandEvent& event, bool write_file, int Pattern  ){
   ...
   // if(!this->m_Lon1->GetValue().ToDouble(&lon1)){ lon1=0.0;}
    ...
    wxString s;
    if (write_file){
        wxFileDialog dlg(this, _("Export demo Positions in GPX file as"), wxEmptyString, defaultFileName, _T("GPX files (*.gpx)|*.gpx|All files (*.*)|*.*"), wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
    ...
    if (write_file){
        ...
        root->SetAttribute("creator", "demo_pi by Rasbats");
    ...
    case 1:
		...
		for (i=0; i<n; i++){	// n is number of routepoints		
			...
			if (total_dist > speed){	
				//
				// demo point is before the next route point
				//
				route_dist = total_dist - myDist;	// route_dist is the distance between the previous demo and the route point
				remaining_dist = speed - route_dist;// distance between route point and next demo
				...
				// Put in the demo point after the route point
				...
				if (myDistForBrng > speed) {
					...
					//
					//
					// All the demo positions inserted for this leg
				}

				if (total_dist == 0) {
					DistanceBearingMercator(latN[i + 1], lonN[i + 1], latF, lonF, &myDistForBrng, &myBrng);
					total_dist = myDistForBrng; // distance between demo and the next route point
					latF = latN[i + 1];
					lonF = lonN[i + 1];
				}
		...
		if ((*itOut).routepoint == 1){
			...
			if ((*itOut).routepoint == 0){
				if (write_file){Addpoint(Route, wxString::Format(wxT("%f"),lati), wxString::Format(wxT("%f"),loni), _T("demo") ,_T("square"),_T("WPT"));}			
			}
		}
